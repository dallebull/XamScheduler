        ...
        async void OnSignUpButtonClicked(object sender, EventArgs e)
        {
            ...
            if (signUpSucceeded)
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    await client.PostAsync("http://localhost:44349/api/accountapi/register", scontent);
        ...
        async Task Login(LoginModel login)
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    var req = new HttpRequestMessage(HttpMethod.Post, "http://localhost:44349/token") { Content = new FormUrlEncodedContent(dictionary) };
